...
import os, io, time, math, json, base64, re, platform
        ...
        if SYSTEM == "Darwin":  # macOS
            ...
            for width_threshold, scaling in retina_patterns:
                if physical_width >= width_threshold * 0.95:  # 5% tolerance
                    print(f"🖥️ Detected macOS display: {physical_width}x{physical_height} → scaling={scaling}")
                    return scaling
            ...
            try:
                ...
                try:
                    ...
                    print(f"🖥️ Windows DPI detection: {dpi_x} DPI → scaling={scaling:.1f}")
...
def poll_result(task_id, max_attempts=20):
    ...
    for attempt in range(max_attempts):
        try:
            response = requests.get(result_url, params={"apikey": HM_API_KEY, "taskId": task_id}, timeout=15)
...
def detect_captcha_type_improved(panel: np.ndarray) -> Tuple[str, float]:
    ...
    print(f"🧩 Detection scores: slide={slide_confidence:.2f}, rotate={rotate_confidence:.2f}, object={object_confidence:.2f}")
...
def detect_slide_captcha_improved(panel: np.ndarray) -> float:
    ...
    for text_area in text_regions:
        if text_area.size > 0:
            ...
            text_thresh = cv2.threshold(text_area, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]
...
def detect_object_selection(panel: np.ndarray) -> float:
    ...
    if 4 <= object_count <= 10:
        ...
        if object_count >= 4:
            ...
            x_coords = [obj[0] + obj[2]//2 for obj in valid_objects]  # center x
            y_coords = [obj[1] + obj[3]//2 for obj in valid_objects]  # center y
    ...
    if top_area.size > 0:
        ...
        text_thresh = cv2.threshold(top_area, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]
...
def solve_slide_captcha(roi_abs, roi_img, region_coords):
    ...
    if res.get("Status") == "SUCCESS" and "Data" in res:
        ...
        print(f"🎯 Kéo từ ({start_x}, {start_y}) đến ({end_x}, {end_y}) (offset: {offset}px)")
        time.sleep(0.9)
...
def detect_captcha_type_by_text(image):
    ...
    try:
        ...
        # Method 1: Ảnh gốc
        text1 = pytesseract.image_to_string(gray, lang='vie+eng', config='--psm 6')
        texts.append(text1.lower())
        
        # Method 2: Threshold
        _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        text2 = pytesseract.image_to_string(thresh, lang='vie+eng', config='--psm 6')
        texts.append(text2.lower())
        
        # Method 3: Chỉ lấy vùng trên (thường có text instruction)
        height = gray.shape[0]
        top_region = gray[0:int(height*0.3), :]
        text3 = pytesseract.image_to_string(top_region, lang='vie+eng', config='--psm 6')
...
def try_all_captcha_types(region_img, region_coords):
    ...
    captured_image_size = actual_image_size  # Pass captured image size to coordinate calculation
...
def calculate_simple_coordinates(x_api, y_api, region_info, captured_image_size=None):
    ...
    if captured_image_size:
        ...
        if captured_w > 1000:
            ...
            print(f"📉 Scaled API coords: ({effective_x:.1f},{effective_y:.1f})")
        else:
    ...
    print(f"📍 Final mapping: ratio=({x_ratio:.3f},{y_ratio:.3f}) → screen=({x_screen},{y_screen})")
...
def execute_captcha_action(captcha_type, data, region_coords, actual_image_size=None, captured_image_size=None):
    ...
    if captcha_type == "slide":
        ...
        print(f"🎯 Slide captcha: API coords=({x_api}, {y_api}), offset={offset}")
        print(f"📐 Region: ({region_x}, {region_y}) size=({region_w}x{region_h})")
        ...
        if actual_image_size:
            ...
            x_ratio = x_api / actual_w
            y_ratio = y_api / actual_h
            offset_ratio = offset / actual_w
            
            x1 = region_x + int(x_ratio * region_w)
            y1 = region_y + int(y_ratio * region_h)
            offset_screen = int(offset_ratio * region_w)
            
            print(f"🎯 Using actual image size: ratio=({x_ratio:.3f}, {y_ratio:.3f}) → screen=({x1}, {y1})")
        else:
            # Detect if we're on Retina display by checking captured image size
            is_retina_scaled = False
            if captured_image_size:
                captured_w, captured_h = captured_image_size
                print(f"📷 Captured image size: {captured_w}x{captured_h}")
                
                # If captured image is > 1000px wide, it's likely 2x scaled (Retina)
                if captured_w > 1000:
                    is_retina_scaled = True
                    print(f"🔍 Detected Retina scaling: image width {captured_w} > 1000")
                else:
                    print(f"🔍 Standard scaling: image width {captured_w} <= 1000")
            
            # Handle Retina vs non-Retina coordinate mapping
            if is_retina_scaled:
                # On Retina, API coordinates are typically 2x scaled
                effective_x = x_api / 2.0
                effective_y = y_api / 2.0
                print(f"🍎 Retina coordinate scaling: API=({x_api},{y_api}) → effective=({effective_x:.1f},{effective_y:.1f})")
            else:
                # On non-Retina, use coordinates directly
                effective_x = x_api
                effective_y = y_api
                print(f"🖥️ Standard coordinates: using API=({x_api},{y_api}) directly")
            ...
            print(f"📍 Coordinate mapping: ratio=({x_ratio:.3f},{y_ratio:.3f}) → screen=({x1},{y1})")
        
        # Calculate offset for slide using proper formula
        # For slide captcha, offset is usually already in pixels
        is_retina_scaled = captured_image_size and captured_image_size[0] > 1000
        
        if is_retina_scaled:
            # 2x scaling detected, scale offset too
            effective_offset = offset / 2.0
            print(f"🍎 Retina offset scaling: {offset} → {effective_offset} (2x scaling detected)")
        else:
            effective_offset = offset
            print(f"📏 Direct offset: {effective_offset}px")
        
        # Convert offset to screen pixels using region width as reference
        if effective_offset > 0:
            # Map offset proportionally to region width
            # Assume API offset is based on standard width (like 716px)
            api_reference_width = 716
            offset_screen = int((effective_offset / api_reference_width) * region_w)
        else:
            offset_screen = region_w // 3  # Default slide distance
        ...
        margin = 10
        if x2 > region_x + region_w - margin:
            x2 = region_x + region_w - margin
            print(f"⚠️ Adjusted x2 to stay within bounds: {x2}")
        elif x2 < region_x + margin:
            x2 = region_x + margin
            print(f"⚠️ Adjusted x2 to stay within bounds: {x2}")
        
        print(f"🖱️ Slide drag: from ({x1}, {y1}) to ({x2}, {y2}) (offset: {offset_screen}px)")
        ...
        if actual_image_size:
            ...
            # Apply Y adjustment for rotate slider positioning
            y_adjustment_pixels = 30  # Move up by 35 pixels for better slider targeting
            
            x1 = region_x + int(x_ratio * region_w)
            y1 = region_y + int(y_ratio * region_h) - y_adjustment_pixels  # Move up
            
            # Ensure Y coordinate doesn't go above region bounds
            if y1 < region_y:
                y1 = region_y + 5  # Keep minimal margin from top
                print(f"⚠️ Y adjusted to stay within region: {y1}")
            
            print(f"🎯 Using actual image size: ratio=({x_ratio:.3f}, {y_ratio:.3f}) → screen=({x1}, {y1})")
            print(f"🔧 Applied Y adjustment: -{y_adjustment_pixels}px to position above slider")
        else:
            # Handle Retina vs non-Retina coordinate mapping
            if is_retina_scaled:
                # On Retina, API coordinates are typically 2x scaled
                # Scale them down by 2 before mapping
                effective_x = x_api / 2.0
                effective_y = y_api / 2.0
                print(f"🍎 Retina coordinate scaling: API=({x_api},{y_api}) → effective=({effective_x:.1f},{effective_y:.1f})")
            else:
            ...
            # Map to screen coordinates using simple proportion
            # Assume API coordinates are in 716x784 space (standard capture size)
            api_reference_w = 716
            api_reference_h = 784
            ...
            # Fix: Adjust Y coordinate for rotate slider positioning
            # Rotate sliders are typically positioned higher than the raw coordinate suggests
            # Apply Y offset adjustment to move the click point up
            y_adjustment_pixels = 30  # Move up by 35 pixels for better slider targeting
        ...
        # Calculate slider width and offset
        # Use actual slider width (typically 60-80% of region width)
        actual_slider_width = region_w * 0.8  # Use 70% of region width as slider width
        
        # For Retina displays, we might need to adjust the offset calculation
        if is_retina_scaled:
            # On Retina, the slider width in the API might be different
            # Use a scaling factor for offset calculation
            offset_base = actual_slider_width / 2.0  # Scale down for Retina
            print(f"🍎 Retina offset base: {actual_slider_width} → {offset_base:.0f}px")
        else:
            offset_base = actual_slider_width
            print(f"🖥️ Standard offset base: {offset_base:.0f}px")
        
        # Calculate offset using the formula: offset = angle * (width_slide/180)
        offset = angle * (offset_base / 180)
        
        print(f"📏 Offset calculation: angle={angle}° × ({offset_base:.0f}/180) = {offset:.1f}px")
        
        # Calculate end coordinates
        x2 = x1 + int(offset)
        y2 = y1  # Y coordinate stays the same (horizontal drag)
        
        # Ensure end position is within reasonable bounds
        margin = 20
        max_x = region_x + int(region_w * 0.85)  # Don't go beyond 85% of region
        ...
        # Additional validation for drag distance
        drag_distance = abs(x2 - x1)
        max_reasonable_drag = actual_slider_width * 0.9  # Max 90% of slider width
        
        if drag_distance > max_reasonable_drag:
            # Scale down the drag if it's too long
            scale_factor = max_reasonable_drag / drag_distance
            new_offset = int(offset * scale_factor)
            x2 = x1 + new_offset
            print(f"🔧 Scaled down drag: {offset:.1f}px → {new_offset}px (factor: {scale_factor:.2f})")
        
        print(f"🖱️ Rotate drag: from ({x1}, {y1}) to ({x2}, {y2})")
        print(f"   📊 Retina: {is_retina_scaled}, Drag: {abs(x2-x1)}px, Max: {actual_slider_width:.0f}px")
        ...
        time.sleep(0.5)
        success = safe_mouse_drag(x1, y1, x2, y2, duration=1.5)  # Longer duration for rotate
        return success
            ...
            for coord_pair in raw_coords.split('|'):
                if ',' in coord_pair:
                    ...
                    if x_api <= 1.0 and y_api <= 1.0:
                        ...
                        print(f"🎯 Object click (ratio): API=({x_api:.1f},{y_api:.1f}) → screen=({x_screen}, {y_screen})")
                    else:
                        ...
                        print(f"🎯 Object click (absolute): API=({x_api:.1f},{y_api:.1f}) → screen=({x_screen}, {y_screen})")
...
def solve_captcha_once():
    ...
    if not FIXED_CAPTCHA_REGION["enabled"]:
        print("❌ Vùng cố định chưa được bật. Vui lòng bật FIXED_CAPTCHA_REGION['enabled'] = True")
        return False
    
    # Lấy thông tin vùng cố định
    region_coords = get_fixed_region_coords()
    print(f"📍 Chụp vùng giữa màn hình: {region_coords[2]}x{region_coords[3]} tại ({region_coords[0]}, {region_coords[1]})")
...
def setup_fixed_coordinates():
    ...
    print(f"🖥️ Monitor hiện tại: {monitor_info['width']}x{monitor_info['height']}")
    print(f"📍 Vùng captcha hiện tại: ({region_coords[0]}, {region_coords[1]}) {region_coords[2]}x{region_coords[3]}")
    print(f"📊 Tỷ lệ hiện tại: x={FIXED_CAPTCHA_REGION['x_percent']:.1%}, y={FIXED_CAPTCHA_REGION['y_percent']:.1%}")
    print(f"📊 Kích thước: w={FIXED_CAPTCHA_REGION['width_percent']:.1%}, h={FIXED_CAPTCHA_REGION['height_percent']:.1%}")
...
def main():
    ...
    if FIXED_CAPTCHA_REGION["enabled"]:
        region_coords = get_fixed_region_coords()
        print(f"📍 Chế độ: Vùng cố định {region_coords[2]}x{region_coords[3]} ở giữa màn hình")
    else:
    ...
    try:
        while True:
            ...
            print(f"📊 {STATS['successes']}/{STATS['attempts']} ({success_rate:.1f}%)")
        ...
        print(f"💰 Chi phí ước tính: ${estimated_cost:.4f} (${STATS['cost_per_thousand']}/1000 lần thành công)")
